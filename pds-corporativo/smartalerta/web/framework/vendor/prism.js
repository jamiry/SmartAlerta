...
(function() {
	...
	function highlightLines(a, b, c) {
		var d = b.replace(/\s+/g, '').split(','), offset = +a
				.getAttribute('data-line-offset') || 0;
		var e = parseFloat(getComputedStyle(a).lineHeight);
		for (var i = 0, range; range = d[i++];) {
			range = range.split('-');
			var f = +range[0], end = +range[1] || f;
	...
	function applyHash() {
		...
		var d = b.slice(0, b.lastIndexOf('.')), pre = document
				.getElementById(d);
		if (!pre) {
			return
		}
...
Prism.hooks.add("after-highlight", function(e) {
	...
	if (t.hasAttribute("data-start")) {
		t.style.counterReset = "linenumber "
